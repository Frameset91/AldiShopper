...
public class GetDataTest extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dialog = ProgressDialog.show(GetDataTest.this, "", "Hier Lädt ein Dings!", true);
		...
		public void run() {
			...
			try {
				...
				for(int i = 0; i<jarr.length();i++){
					...
					tmpObj.getString("productID");
					tmpObj.getString("name");
					tmpObj.getString("categoryID");
					tmpObj.getString("cdesc");
					tmpObj.getString("description");
					tmpObj.getString("unit");
					tmpObj.getString("price");
...
//					Log.i("Debug mich!!! "+i, tmpObj.getString("productID"));
	...
	private Runnable showUpdate = new Runnable() {
		...
		private JSONArray getJsonArray(String URL) {
			...
			try {
				json = (new JSONArray(JsonResponse));	// Convert String to JSON (screws Ordering of JSON)
			} catch (Exception e) {
				//Log.e("##############################################", "UNIverse getJson Exception: "+URL);
			}
		...
		public static String connect(String url, List<NameValuePair> postParams) {
			...
			try {
				...
				//Log.e("##############################################", "UNIverse exception2: "+url);
			} catch (IOException e) {
				//Log.e("##############################################", "UNIverse exception3: "+url);
			}
			return null;
		}
